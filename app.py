...
if st.session_state.stage == 1:
    ...
    user_input = st.text_input("Decrypt key word payload:", key="in_1", placeholder="Type response here...").strip()
    
    if st.button("EXECUTE HANDSHAKE UNLOCK 🔓", key="b_1"):
        if check_hash(user_input, "2f8263cf8bb82ec7da117c2be6d11a8080f83359d48b1116c905327b73840e79"): # Hashed "POWER"
...
# =========================================================================
    ...
    user_input = st.text_input("Enter the terminal bit registry integer output value (0 or 1):", key="in_2")
    
    if st.button("VERIFY VOLTAGE LOGIC 🔓", key="b_2"):
        if check_hash(user_input, "5feceb66ffc86f38d952786c6d696c79c2dbc239dd4e91b46729d73a27fb57e9"): # Hashed "0"
...
# =========================================================================
    ...
    user_input = st.text_input("Translate extracted target string sequence:", key="in_3")
    
    if st.button("BYPASS MEMORY FIREWALL 🔓", key="b_3"):
        if check_hash(user_input, "9a661b0a52df03aa6240292797e88f00072b49c001cbe3d007ec1dfb1b60f588"): # Hashed "CODE"
...
# =========================================================================
    ...
    user_input = st.text_input("Enter the single character token symbol missing from lines 1 and 2:", key="in_4")
    
    if st.button("COMPILE SYNTAX OVERRIDE PATCH 🔓", key="b_4"):
        if check_hash(user_input, "76f0d14b4369a473468087920bb6148303f8fcb151bf99994c979cf7b198889a"): # Hashed ":"
...
# =========================================================================
# CHALLENGE 5: SQL DATA MODEL EXCLUSION (NEW CHALLENGE)
# =========================================================================
    ...
    user_input = st.text_input("Complete the missing SQL keyword pattern string:", key="in_5", placeholder="Keyword...").strip()
    
    if st.button("EXECUTE STRUCTURED DATA QUERY 🔓", key="b_5"):
        if check_hash(user_input, "601e3895bd97ff645e9987a0709b456db97f8c057692095b6a71e1f74463402e"): # Hashed "DISTINCT"
...
# =========================================================================
# CHALLENGE 6: SUBNET ROUTING RESOLUTION (NEW CHALLENGE)
# =========================================================================
    ...
    user_input = st.text_input("Enter the 3-letter acronym designator string:", key="in_6", placeholder="e.g. FTP, SSL...").strip()
    
    if st.button("TERMINATE ROOT ROUTING HANDSHAKE 🔓", key="b_6"):
        if check_hash(user_input, "e5b72195f22e700cf1cb765f606e10885e35e7df5d1bf25577609278065c711a"): # Hashed "DNS"
